...
def main(**kwargs):
    ...
    # Load the tokenizer and add special tokens if necessary
    ...
    if train_config.enable_fsdp:
        ...
        mixed_precision_policy, wrapping_policy = get_policies(
            fsdp_config, rank)
        # Dynamically load the layer class for FSDP auto-wrapping
        if train_config.use_peft:
            if hasattr(train_config, 'fsdp_auto_wrap_module'):
                module_path, class_name = train_config.fsdp_auto_wrap_module.rsplit(
                    '.', 1)
                ...
                raise ValueError(
                    "fsdp_auto_wrap_module must be specified in config when using PEFT with FSDP")
        else:
